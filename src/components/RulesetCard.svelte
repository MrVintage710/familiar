...
<script lang="ts">
  ...
  const imageUri = () => 'data:image/gif;base64,' + ruleset.covers[Math.floor(Math.random() * ruleset.covers.length)].data
</script>
...
<div style="--image-data : url({imageUri()})" class="container relative w-full card preset-filled-surface-100-900 flex justify-center p-2 darken h-fit">
  <Button onclick={() => {}} class="absolute right-2 drop-shadow-[0_2.2px_2.2px_rgba(0,0,0,0.8)] m-1">
