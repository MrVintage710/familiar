<script lang="ts" module>
  ...
  import NewCharacterPage from "$lib/page/NewCharacterPage.svelte";
...
<script lang="ts">
  ...
  const page = () => new NewCharacterPage(ruleset);
  
  console.log("Header", page().getUrl());
...
<div style="--image-data : url({imageUri()})" class="container relative w-full card preset-filled-surface-100-900 flex justify-center p-2 darken h-fit">
  <Button onclick={() => addPage(new NewCharacterPage(ruleset))} class="absolute right-2 drop-shadow-[0_2.2px_2.2px_rgba(0,0,0,0.8)] m-1">
